...
/* Lluvias.
El programa permite calcular el promedio mensual de las lluvias caídas en tres regiones importantes del país.
Determina también cuál es la región con mayor promedio de lluvia anual. */
...
void main(void)
{
    ...
    printf("\nPromedio de lluvia Region Pacifico: %6.2f ", (APAC / 12));
    printf("\nPromedio de lluvias Region Caribe: %6.2f\n", (ACAR / 12));
    Mayor(AGOL, APAC, ACAR);
    /* Se llama a la función MMaayyoorr. Paso de parámetros por valor. */
}
...
/* Esta función obtiene la región con mayor promedio de lluvia anual. */
{
    if (R1 > R2)
        if (R1 > R3)
        printf("\nRegion con mayor promedio: Region Golfo. Promedio: %6.2f", R1 / R2);
    else
